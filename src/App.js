...
export default function App() {
  ...
  const openSellModal = (stock) => {
    setSellModal({
      ...
      sharesToSell: stock.shares, // 預設帶入全部股數
      sellPrice: livePrices[stock.ticker] || stock.buyPrice, // 預設帶入當前現價
      commission: "", // 手續費留給使用者填寫
    });
  };
  ...
  const fetchRealTimePrices = async () => {
    ...
    try {
      ...
      // 由於 Finnhub 免費版/quote API 通常是一次查一檔股票，這裡用 Promise.all 並行請求
      await Promise.all(
        tickers.map(async (ticker) => {
          try {
            ...
            // Finnhub 回傳的物件中，'c' 代表 Current price (現價)
    ...
    <div className="min-h-screen bg-[#0d0e12] text-slate-300 p-4 md:p-8 font-sans relative">
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-wrap gap-2 bg-[#1a1c23]/50 p-1.5 rounded-2xl border border-slate-800 w-fit">
          {[
            { id: "combined", label: "綜合持股", icon: <Users size={16} /> },
            { id: "me", label: "我的", icon: <User size={16} /> },
            { id: "dad", label: "爸爸", icon: <User size={16} /> },
            { id: "cash", label: "現金管理", icon: <DollarSign size={16} /> },
            { id: "history", label: "交易日誌", icon: <History size={16} /> },
          ].map((tab) => (
            <button
              key={tab.id}
